...
const ex_00 = () => { // незахватывающие группы
    ...
    // не попадает в результаты (в массив)
...
const ex_01 = () => { // незахватывающие группы
    ...
    // for (let m of str.matchAll(re)) { log(m); }
    
    log(1.1, [...str.matchAll(re)].map(m => m[1])); 
    // ["regex101", "youtube"]
    re = /(?:https?|ftp):\/{2}(?:www\.)?(.+)(?:\.[A-ZА-ЯЁ]{2,}\b)/gi; // ???
    // re = /(?:https?|ftp):\/{2}(?:www\.)?(.+)(?:\.com)/g;
...
const ex_02 = () => { // группы + обратные ссылка (backreference)
    ...
    let str = `2026 666 ASA 11 1024 1606 27072 1230341`;

    // нумерованная группа - это \цифра
    let re = /((\d{1})\d{1}\2)/g; // 232  66066
    log(2.1, [...str.matchAll(re)].map(m => m[1])); 
    // [ '202', '666', '606', '707', '303' ]

    re = /(?:\d)((\d)\d\2)(?:\d)/g; // 46067
