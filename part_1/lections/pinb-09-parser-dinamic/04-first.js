...
(async () => {
    ...
    await page.waitForSelector('.tab-content', { timeout: 5000 }); // подождём чтобы прогрузилась страница
    // не ждёт всё время - только пока грузится    
    const firstBlock = await page.$('.tab-content'); // найти первый блок

    const result = await firstBlock
        .evaluate(el => { 
            return {
