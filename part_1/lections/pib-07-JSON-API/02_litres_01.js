...
let type = 'text_book';
let params = `is_for_pda=false&limit=${limit}&offset=0&q=${prodFindEncoded}&show_unavailable=false&types=${type}`;
let url = `${host}?${params}`;
let json = getProducts(url).payload.data;

let strJSON = JSON.stringify(json, null, 2);
require('fs').writeFileSync('./files/products.json', strJSON, 'utf8');
...
limit=24 &
offset=0 & 
q=%D0%B3%D0%B5%D0%B3%D0%B5%D0%BB%D1%8C &
show_unavailable=false & 
types=text_book&types=audiobook&types=podcast&types=podcast_episode&types=webtoon
